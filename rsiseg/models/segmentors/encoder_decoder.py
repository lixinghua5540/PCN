...
class EncoderDecoder(BaseSegmentor):
    ...
    def simple_test(self, img, img_meta, rescale=True):#the first function to conduct
    ...
    def simple_test_feature(self, img, img_meta, rescale=True):#feature level result extraction 
        """Simple test with single image."""
        seg_logit, states = self.inference(img, img_meta, rescale)#batch, class, h, w
        #seg_pred = seg_logit.argmax(dim=1)
        if torch.onnx.is_in_onnx_export():
            # our inference backend only support 4D output
            seg_pred = seg_pred.unsqueeze(0)
        ...
        return seg_logit, state_list#store the state logits to cur_state?
