...
class CustomDataset(Dataset):
    ...
    def prepare_test_img(self, idx):#prepare test img
        """Get testing data after pipeline.
...
@DATASETS.register_module()
class CustomDatasetCopy(Dataset):
    """Custom dataset for semantic segmentation. An example of file structure
    is as followed.
    ...
    def load_annotations(self, img_dir, img_suffix, ann_dir, seg_map_suffix,
        ...
        if split is not None:
            ...
            for line in lines:
                ...
                img_info = dict(filename=img_name + img_suffix)
                if ann_dir is not None:
                    seg_map = img_name + "_label" + seg_map_suffix#modify the expression here
                    img_info['ann'] = dict(seg_map=seg_map)
                img_infos.append(img_info)
        else:
            for img in self.file_client.list_dir_or_file(
                ...
                if ann_dir is not None:
                    seg_map = img.replace(img_suffix, seg_map_suffix)
                    #add some expression
                    seg_map = seg_map.split(seg_map_suffix)[0]+'_label'+seg_map_suffix
                    img_info['ann'] = dict(seg_map=seg_map)
                img_infos.append(img_info)
            img_infos = sorted(img_infos, key=lambda x: x['filename'])
