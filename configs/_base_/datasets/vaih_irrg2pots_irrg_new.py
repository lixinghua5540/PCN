...
target_pipeline = [
    ...
    dict(type='LoadAnnotationsPseudoLabelsV2',
         ...
         load_feats=False,
         pseudo_ratio=0.0),
    dict(type='Resize', img_scale=(576, 576), ratio_range=(0.5, 2.0)),#1024
    dict(type='RandomCrop', crop_size=crop_size, cat_max_ratio=0.75),
    dict(type='RandomRotate90', prob=1.0),
    dict(type='RandomFlip', flip_ratio=0.5, direction='vertical'),
    dict(type='RandomFlip', flip_ratio=0.5, direction='horizontal'),
...
gt_seg_map_loader_cfg=dict(reduce_zero_label=True)#The original label has 0 value denotes unknown class
data = dict(
    samples_per_gpu=4,#2
    workers_per_gpu=4,
    train=dict(
