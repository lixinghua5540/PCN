...
class LoadImageFromFile(object):
    ...
    def __call__(self, results):
        ...
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = img
        results['ori_img'] = img#keep the original image to realize image level translation
        results['img_shape'] = img.shape
        results['ori_shape'] = img.shape
        # Set initial values for default meta_keys
        results['pad_shape'] = img.shape
...
@PIPELINES.register_module()
class Load16bitImageFromFile(object):
    """Load a 16-bit image from file with proper bit depth handling.
    ...
    Args:
        to_float32 (bool): Whether to convert the loaded image to a float32
            numpy array. If set to False, the loaded image retains its original
            bit depth. Defaults to False.
        color_type (str): The flag argument for image loading. 
            Defaults to 'color'.
        file_client_args (dict): Arguments to instantiate a FileClient.
            Defaults to ``dict(backend='disk')``.
    """
    ...
    def __call__(self, results):
        ...
        # Directly read file for better 16-bit handling
        ...
        # Ensure 16-bit data is properly handled
        if img.dtype == np.uint16:
            if self.to_float32:
                img = img.astype(np.float32) / 65535.0  # Normalize to 0-1
            else:
                # Keep as uint16 but check value range
                if img.max() > 65535 or img.min() < 0:
                    raise ValueError("16-bit image values out of range")
        elif img.dtype == np.uint8 and not self.to_float32:
            img = img.astype(np.uint16) * 256  # Scale 8-bit to 16-bit

        # Convert 2-channel to 3-channel by duplicating last channel
        if len(img.shape) == 3 and img.shape[2] == 2:
            img = np.concatenate([img, img], axis=2)
        # NEW: Handle 4-channel images (e.g. RGBA) modified
        # if len(img.shape) == 3 and img.shape[2] == 4:
        #     img = img[:, :, :3]  # Keep only first 3 channels
            #print(f"Warning: Dropped alpha channel from 4-channel image: {filename}")
        results['filename'] = filename
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = img
...
class LoadImageFromFile_forAdap(object):
    ...
    def __call__(self, results):
        ...
        if not 'img_fields' in results:
            results['img_fields'] = []
        results['img_fields'].append('img')
        #results['img_fields'].append('B_img')#was not displayed
        return results
