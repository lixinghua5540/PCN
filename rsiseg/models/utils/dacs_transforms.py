...
def weak_transform(param, data=None, target=None):
    assert ((data is not None) or (target is not None))
    if 'mix' in param:
        data, target = one_mix(mask=param['mix'], data=data, target=target)
    # data, target = color_jitter(
    ...
    #     denorm_type=param['denorm_type']
    # )
    # data, target = gaussian_blur(blur=param['blur'], data=data, target=target)#this may make it better
    return data, target
...
def get_mean_std(img_metas, dev):
    ...
    length = len(mean)#the logic has something wrong here
...
def get_mean_std_4bands(img_metas, dev):
    mean = [
        torch.as_tensor(img_metas[i]['img_norm_cfg']['mean'], device=dev)
        for i in range(len(img_metas))
    ]
