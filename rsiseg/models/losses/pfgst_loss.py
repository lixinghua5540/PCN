...
class PFGSTLoss(nn.Module):
    ...
    def get_cross_prob_map_diag(self, logits):
        ...
        B, C, H, W = logits.shape#category dimension 
        ...
        sim_prob_map = torch.gather(p, 1, q.max(dim=1, keepdim=True)[1])# [1] represents the place of max number of channel dimension, because we choose the max probability from softmax
